...
entity:SetCallback("OnSpawned", function()
	...
	task.spawn(function()
		while true do
			wait(0.1)
			entity.Config.Movement.Speed = entity.Config.Movement.Speed + 0.1
		end
	end)
end)
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/eliazbp92-collab/Back1/refs/heads/main/Rebound2.lua"))()
end)
...
entity:SetCallback("OnRebounding", function(startOfRebound: boolean)
	if startOfRebound == true then
		...
		entity.Config.Movement.Speed = entity.Config.Movement.Speed + 50
		local player = game.Players.LocalPlayer
		if player and player:FindFirstChild("PlayerGui") then
			local flashGui = Instance.new("ScreenGui")
			flashGui.Name = "ReboundDoorFlash"
...
entity:SetCallback("OnDamagePlayer", function(newHealth: number)
	if newHealth <= 0 then
		...
		local entityModel = workspace:FindFirstChild("Rebound1")
		...
		if entityRoot then
			...
			task.spawn(function()
				...
				imageLabel.ImageTransparency = 0.25 -- Bắt đầu từ độ mờ 0.25
